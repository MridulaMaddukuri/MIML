...
def get_train_val_split_data(
    root, transform=None, val_size=0.33, seed=1, num_workers=4
):
...
class MIMLBagsData(Dataset):
    ...
    def _create_bags(self):
        ...
        for i in tqdm(range(self.num_bag)):
            ...
            # pick a random number indicating how many categories
            # to sample instances from
            ...
            for sc in sampled_categories:
                ...
                # print(f"Population: {len(label_img_dict[sc])}
                # and Sample {num_instances}")
